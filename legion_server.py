...
def Run(args):
    ...
    with open("meta_config", "w") as file:
        file.write("{} {} {} {} {} {} {} {} {} {}".format(
            path, args.train_batch_size, vertices_num, edges_num, features_dim,
            train_set_num, valid_set_num, test_set_num, args.cache_memory,
            args.epoch))
    ...
    if args.usenvlink == 1:
        ...
        group_size, fully_connected_groups = find_largest_fully_connected_group(
            G)
        if fully_connected_groups or group_size == 1:
            print(
                f"NVLink clique size: {group_size}, Number of NVLink cliques: {int(gpu_number/group_size)}"
            )
        cache_agg_mode = math.log2(group_size)
    else:
        cache_agg_mode = 0

    os.system("./sampling_server/build/bin/sampling_server {} {} {}".format(
        gpu_number, cache_agg_mode, args.fanout))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--fanout', type=str, default="12,12,12")
