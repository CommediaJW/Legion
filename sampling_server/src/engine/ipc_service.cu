...
public:
  ...
  // compute number of training, validation, testing iterations
  ...
  int32_t GetLocalBatchId(int32_t global_batch_id) override
  {
    ...
    if ((global_batch_id) < ((train_step_ + valid_step_) * epoch_))
    { // train & valid
  ...
  {
    if (current_mode == TRAINMODE)
    { // train
      return train_batch_size_[dev_id];
    }
    else if (current_mode == VALIDMODE)
    { // valid
